...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="bg-deep-purple text-white shadow-md sticky top-0 z-40 border-b border-purple-900/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate('landing')}>
          ...
          <div>
            <h1 className="text-base md:text-lg font-black tracking-tight flex items-center gap-2">
              ...
              <span className="text-orange-300 text-xs font-normal hidden lg:inline">
        ...
        {/* Center Nav Links (4 items) */}
        <nav className="hidden md:flex items-center gap-1 bg-purple-950/60 p-1 rounded-2xl border border-purple-800/40">
          {navItems.map((item) => {
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                  isActive
                    ...
                    : 'text-purple-200/90 hover:text-white hover:bg-white/10'
                }`}
              >
      ...
      <div className="md:hidden bg-deep-purple border-t border-purple-900/50 flex items-center justify-around py-2 px-2">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeScreen === item.id;
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center p-1.5 rounded-xl text-[10px] font-semibold transition-all min-w-[56px] cursor-pointer ${
                isActive ? 'text-bright-orange font-bold' : 'text-purple-200 hover:text-white'
              }`}
            >
