...
export const SlideDeckViewer: React.FC<SlideDeckViewerProps> = ({ slideDeck, onToggleApproval }) => {
  ...
  return (
    <div className="bg-white rounded-3xl p-5 border-2 border-teal-500 shadow-sm flex flex-col justify-between space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-teal-950">PowerPoint Slide Deck</h3>
      ...
      {activeSlide && (
        <div className="bg-teal-900 text-white rounded-2xl p-4 flex flex-col justify-between min-h-[220px] shadow-inner relative overflow-hidden">
          {/* Slide Header */}
          ...
          <div className="my-3">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs text-teal-100">
          ...
          <div className="bg-teal-950/80 p-2.5 rounded-xl border border-teal-800 text-[10px] text-orange-200 flex items-start gap-1.5">
          ...
          {/* Speaker Notes */}
          <p className="text-[10px] text-teal-300 italic mt-2 border-t border-teal-800/60 pt-1.5">
            <span className="font-bold text-orange-300">Teacher Notes:</span> {activeSlide.speakerNotes}
          </p>

          {/* Navigation Arrows */}
          <div className="flex justify-between items-center mt-3 pt-2 border-t border-teal-800">
            <button
              onClick={handlePrev}
              disabled={currentSlideIndex === 0}
              className="p-1 rounded-lg bg-teal-800 text-teal-200 hover:bg-teal-700 disabled:opacity-30 cursor-pointer"
            >
              ...
              {slides.map((_, idx) => (
                <span
                  ...
                  className={`w-2 h-2 rounded-full cursor-pointer ${
                    currentSlideIndex === idx ? 'bg-orange-400' : 'bg-teal-800'
                  }`}
                />
              ))}
            </div>
