...
export const ActiveReviewPanel: React.FC<ActiveReviewPanelProps> = ({
  ...
  return (
    <div className="col-span-1 lg:col-span-2 bg-[#3B176B] rounded-3xl p-6 text-white flex flex-col gap-4 shadow-xl border border-purple-800">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-purple-800/80 pb-4">
        <div className="flex items-center gap-3">
          <span className="text-orange-300 text-xl font-bold">⚡ Active Review:</span>
          <h3 className="text-lg font-bold text-white">{title}</h3>
          <span
            className={`text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider ${
              isApproved ? 'bg-bright-orange text-white' : 'bg-orange-500 text-white'
            }`}
          >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-[11px] font-bold bg-white/10 hover:bg-white/20 text-purple-100 px-3 py-1.5 rounded-xl transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-xl text-xs font-black transition-all shadow-sm flex items-center gap-1.5 cursor-pointer ${
              isApproved
                ? 'bg-bright-orange hover:bg-orange-600 text-white'
      ...
      <div className="bg-[#201A2B] rounded-2xl p-4 md:p-5 border border-purple-800 space-y-4 max-h-[420px] overflow-y-auto text-xs text-purple-100 leading-relaxed font-sans">
        {sectionKey === 'lessonPlan' && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-purple-200">{lessonPackage.lessonPlan.materialsNeeded.join(', ')}</p>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(lessonPackage.lessonPlan.deliverySteps).map(([stepKey, stepValRaw]) => {
                  ...
                  return (
                    <div key={stepKey} className="bg-purple-900/80 p-3 rounded-xl border border-purple-800">
                      <div className="flex justify-between items-center mb-1 font-bold text-white uppercase text-[10px] tracking-wider">
                        <span className="text-orange-300">{stepKey.replace(/([A-Z])/g, ' $1')}</span>
                        <span className="bg-purple-800 px-2 py-0.5 rounded text-purple-200">{stepVal.duration}</span>
                      </div>
                      <p className="text-purple-200"><span className="font-bold text-white">Teacher:</span> {stepVal.teacherActivity}</p>
                      <p className="text-purple-300 mt-0.5"><span className="font-bold text-purple-100">Pupil:</span> {stepVal.pupilActivity}</p>
                    </div>
                  );
                })}
              </div>
        ...
        {sectionKey === 'teacherNotes' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-purple-100 leading-relaxed">{lessonPackage.teacherNotes.pedagogicalBackground}</p>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {lessonPackage.teacherNotes.commonMisconceptions.map((mc, idx) => (
                  <div key={idx} className="bg-purple-900/80 p-3 rounded-xl border border-orange-500/30">
                    <p className="text-orange-300 font-bold mb-1">⚠️ Misconception: "{mc.misconception}"</p>
                    <p className="text-purple-100">💡 Correction Strategy: {mc.correctionStrategy}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {sectionKey === 'learnerNotes' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-purple-100 text-sm leading-relaxed">{lessonPackage.learnerNotes.summaryText}</p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {lessonPackage.learnerNotes.keyVocabulary.map((vocab, i) => (
                  <div key={i} className="bg-purple-900/80 p-2.5 rounded-xl border border-purple-800">
                    <span className="font-bold text-orange-300 text-xs block mb-0.5">{vocab.term}</span>
                    <span className="text-purple-200 text-[11px]">{vocab.definition}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {sectionKey === 'activity' && (
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-purple-200">{lessonPackage.activity.localMaterials.join(', ')}</p>
            </div>
        ...
        {sectionKey === 'quiz' && (
          <div className="space-y-3">
            ...
            {lessonPackage.quiz.questions.map((q, i) => (
              <div key={i} className="bg-purple-900/80 p-3 rounded-xl border border-purple-800 space-y-1">
                <p className="font-bold text-white">{q.id}. {q.questionText}</p>
                {q.options && (
                  <div className="grid grid-cols-2 gap-1 my-1 text-[11px] text-purple-200">
                ...
                <p className="text-purple-300 text-[10px]">Explanation: {q.explanation}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {sectionKey === 'worksheet' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-purple-200 italic mb-2">{lessonPackage.worksheet.instructions}</p>
              <ul className="list-disc list-inside space-y-1 text-purple-100">
            ...
            <div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-[10px] text-purple-200 border-collapse">
                  <thead>
                    <tr className="bg-purple-950 text-orange-300 font-bold border-b border-purple-800">
